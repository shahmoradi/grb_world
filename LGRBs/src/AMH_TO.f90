...
SUBROUTINE AMH(npar,iter,ireport,idelay,X,COV,SR,func,LABEL)
  IMPLICIT NONE
  INTEGER, INTENT(IN) :: npar,iter,ireport,idelay  
  ...
  open(unit=1112,file='AMHMCMC_SAMPLE.txt')  
  open(unit=1999,file='MCMC_Time.txt')  
  write(1999,'(5A25)') 'iteration','avg_progressive_accr','avg_time','time_taken_sofar','time_remained'
  labels='(   A30)'
  write(labels(2:4),'(I3)') npar+4
  write(1112,labels) 'accepted_iter','iteration','Log(Likelihood)','avg(accr)',(TRIM(LABEL(j)),j=1,npar)
  ...
  accr=0.d0/0.d0  
  write(1112,string) i,j,loglike_current,accr,(X(k),k=1,npar)
  sumaccr=0.0d0
  ...
  InitialMarkovChain: do      
    ...
    if (ran2(idum)<accr) then  
      ...
      write(1112,string) i,j,loglike_current,sumaccr/dble(j),(X0(i,k),k=1,npar)
    end if
    if (jj==ireport) then
      CALL CPU_TIME(tend)
      ...
      write(1999,'(1I25,4E25.8)') j,sumaccr_ireport/dble(ireport),tend-tbegin,tend-tbegin0,dble(iter-i)*(tend-tbegin0)/dble(i)
      tbegin=tend
      jj=0
      sumaccr_ireport=0.0d0
    end if
  ...
  write(*,*) 'Now starting the Adaptive MH MCMC sampling...'
    scalefactor=0.8*2.38**2/dble(npar)  
    MEAN=0.d0
    SUMX0X0=0.d0
    do ii=1,idelay
  ...
  MarkovChain: do
    ...
    if (ran2(idum)<accr) then  
      ...
      X=PROPOSAL
      loglike_current=loglike_proposal
      write(1112,string) i,j,loglike_current,sumaccr/dble(j),(X(k),k=1,npar)
    ...
    if (jj==ireport) then
      ...
      write(*,'(A,I5,A,1F15.6,A)') 'Execution time for the last ',ireport,' iterations: ',tend-tbegin,' seconds.'
      write(*,'(A,1E15.6,A)') 'Elapsed time since the beginning of Markov Chain: ',tend-tbegin0,' seconds.'
      write(1999,'(1I25,4E25.9)') j,sumaccr_ireport/dble(ireport),tend-tbegin,tend-tbegin0,dble(iter-i)*(tend-tbegin0)/dble(i)
      tbegin=tend
      jj=0
      sumaccr_ireport=0.0d0
    end if
